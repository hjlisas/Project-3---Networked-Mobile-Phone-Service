...
// **   Filename: PrepaidHandler.java 1.0 02/09/13  **
// **   hjlisas, admu 2002                              **
// *******************************************************
...
{
	Connection c = null;
	PreparedStatement addSubscriber, getSubscribers,
					  getBalance, updateBalance, all;
	...
	public void addSubscriber( String ID, String firstName, String lastName, double balance )
	{
		try
		{
			...
			addSubscriber = c.prepareStatement( "INSERT INTO PREPAID VALUES(?,?,?,?)" );
	...
	public double getBalance( String ID )
	{
		...
		{
			c = DriverManager.getConnection( "jdbc:odbc:MobileDB" );
			getBalance = c.prepareStatement( "SELECT * FROM PREPAID WHERE ID=?" );
	...
	public void updateBalance( String ID, double value )
	{
		...
		{
			c = DriverManager.getConnection( "jdbc:odbc:MobileDB" );
			updateBalance = c.prepareStatement( "UPDATE PREPAID SET BALANCE = " + temp + " WHERE ID = ?" );
	...
	public Vector getAll()
	{
		...
		{
			c = DriverManager.getConnection( "jdbc:odbc:MobileDB" );
			getSubscribers = c.prepareStatement( "SELECT * FROM PREPAID" );
	...
	public Vector getAllSubscribers()
	{
		...
		{
			c = DriverManager.getConnection( "jdbc:odbc:MobileDB" );
			all = c.prepareStatement( "SELECT * FROM PREPAID" );
			rs = all.executeQuery();
			while( rs.next() )
			{
